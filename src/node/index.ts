...
export class NodeVenv {
    ...
    private runInstallNodeCommand(version: string, type: InstallType) {
        if(this.isWindows) {
            try {
                ...
                execSync(`powershell -Command "${type} install ${version}"`, { stdio: 'inherit', windowsHide: true });
                execSync(`powershell -Command "${type} use ${version}"`, { stdio: 'inherit', windowsHide: true });
            } catch (error) {
                logger.error(`Failed to install Node.js with ${type} on Windows. Make sure ${type} is installed and the version is valid.`);
                process.exit(1);
            }
        }
        else {
    ...
    public updateNvmrcFile(version: string, type:InstallType = 'nvm'): void {
        ...
        try {
            ...
            logger.error(`Failed to update ${nvmrcPath} file. Make sure you have the necessary permissions.`);
        }
    }
    ...
    public autoManageNode() {
        ...
        try {
            if (!fs.existsSync(nvmrcPath)) {
                logger.error(`No ${nvmrcPath} file found in the current directory. Please create one with a Node.js version.`);
                process.exit(1);
            }
